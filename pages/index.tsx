  ...
  return (
    <div className="bg-black" style={{ scrollSnapType: "y proximity" }}>
      ...
      {(() => {
        ...
        // Build one "half" that's wide enough to fill the screen, then duplicate it
      ...
      <section id="hero" className="relative flex flex-col overflow-hidden" style={{ height: "100dvh", scrollSnapAlign: "start" }}>
        ...
        <div className="relative z-10 bg-[#9945ff] h-16 overflow-hidden shrink-0">
          {(() => {
            ...
            return (
              <div className="flex w-max" style={{ animation: "partnerScroll 30s linear infinite" }}>
                <div className="flex shrink-0">{logos.map((p, i) => renderItem(p, i))}</div>
                <div className="flex shrink-0">{logos.map((p, i) => renderItem(p, i + logos.length))}</div>
              </div>
            );
          })()}
        </div>
