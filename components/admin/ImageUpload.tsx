...
import { Upload, X, Image as ImageIcon, Crop, Pencil, ZoomIn, Eraser } from "lucide-react";
...
interface ImageUploadProps {
  ...
  folder?: string;
  noCrop?: boolean;
  /** For logo editor mode: current scale value */
  scaleValue?: number;
  /** For logo editor mode: callback when scale changes */
...
export function ImageUpload({ value, onChange, bucket, folder, noCrop, scaleValue, onScaleChange }: ImageUploadProps) {
  const [uploading, setUploading] = useState(false);
  const [dragOver, setDragOver] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);

  // Cropper state (for non-noCrop mode)
  ...
  // Logo editor state (for noCrop mode)
  const [editorOpen, setEditorOpen] = useState(false);
  const [editorSrc, setEditorSrc] = useState<string | null>(null);
  const [editorFile, setEditorFile] = useState<File | null>(null);
  const [editorScale, setEditorScale] = useState(scaleValue ?? 1);
  ...
  // ─── Logo Editor (noCrop mode) ───
  function openEditor(file: File) {
    const url = URL.createObjectURL(file);
    setEditorSrc(url);
    setEditorFile(file);
    setEditorScale(scaleValue ?? 1);
    setEditorOpen(true);
  }

  function openEditorExisting() {
    if (!value) return;
    setEditorSrc(value);
    setEditorFile(null);
    setEditorScale(scaleValue ?? 1);
  ...
  function closeEditor() {
    if (editorSrc && editorSrc.startsWith("blob:")) URL.revokeObjectURL(editorSrc);
    setEditorSrc(null);
    setEditorFile(null);
    setEditorOpen(false);
  }
  ...
  async function handleEditorSave() {
    // Save scale
    onScaleChange?.(editorScale);
    // Upload file if it's a new file
    if (editorFile) {
      closeEditor();
      await uploadFile(editorFile);
    } else {
      closeEditor();
    }
  }

  async function handleEditorRemoveBg() {
    if (!editorSrc) return;
    setRemovingBg(true);
    try {
      const blob = await removeBackground(editorSrc);
      // Replace editor source with the transparent version
  ...
  async function handleCropConfirm() {
    if (!cropSrc || !croppedArea) { toast.error("No crop area selected"); return; }
    try {
      ...
      if (pendingFile) { closeCropper(); await uploadFile(pendingFile); }
    }
  }
  ...
  function handleRemove() { onChange(""); }
    ...
    <div className="space-y-2">
      ...
      {value && (
        <div className="relative inline-block">
          <img src={value} alt="Preview" className="w-20 h-20 rounded-lg object-cover border border-border-subtle" />
          {noCrop ? (
            <button
              type="button"
              onClick={openEditorExisting}
      ...
      <div
        ...
        className={`flex items-center gap-3 px-4 py-3 rounded-lg border border-dashed cursor-pointer transition-colors ${
          dragOver ? "border-brand-purple bg-brand-purple/5" : "border-border-subtle hover:border-brand-purple/50"
        }`}
      >
        {uploading ? (
      ...
      {editorOpen && editorSrc && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/70" onClick={closeEditor} />
          <div className="relative w-full max-w-lg bg-bg-card border border-border-subtle rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-border-subtle">
              ...
              <button onClick={closeEditor} className="p-1 text-text-muted hover:text-text-primary">
                <X size={16} />
              </button>
            </div>

            {/* Preview area — matches the actual purple bar (h-16 = 64px) */}
            <div className="relative w-full flex items-center justify-center bg-[#9945ff]" style={{ height: 64 }}>
              <img
                src={editorSrc}
                alt="Logo preview"
                className="object-contain brightness-0 invert opacity-90"
                style={{ height: "1.75rem", maxWidth: "120px", transform: `scale(${editorScale})`, transition: "transform 0.15s ease" }}
              />
            </div>
            ...
            <div className="px-4 py-3 border-t border-border-subtle space-y-3">
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs text-text-muted shrink-0">Size</span>
                <input
                  type="range"
                  min={0.5}
                  max={4}
                ...
                />
                <span className="text-xs font-mono text-text-primary w-8 text-center">{editorScale.toFixed(1)}</span>
              </div>
              ...
              <div className="flex items-center justify-between">
                <button
                  type="button"
                  onClick={handleEditorRemoveBg}
                  disabled={removingBg}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-bg-elevated text-text-secondary rounded-lg hover:text-text-primary hover:bg-bg-card-hover transition-colors disabled:opacity-50"
                >
                  <Eraser size={12} />
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={uploading}
                    className="px-4 py-2 text-xs font-semibold bg-brand-purple text-white rounded-lg hover:bg-brand-purple-light transition-colors disabled:opacity-50"
                  >
          ...
          <div className="relative w-full max-w-lg bg-bg-card border border-border-subtle rounded-xl overflow-hidden">
            ...
            <div className="px-4 py-3 border-t border-border-subtle">
              <div className="flex items-center gap-3 mb-3">
                ...
                <input type="range" min={1} max={3} step={0.05} value={zoom} onChange={(e) => setZoom(Number(e.target.value))} className="flex-1 accent-brand-purple" />
              </div>
              <div className="flex justify-end gap-2">
                <button onClick={pendingFile ? handleSkipCrop : closeCropper} className="px-4 py-2 text-xs font-medium text-text-secondary hover:text-text-primary transition-colors">
                  {pendingFile ? "Skip Crop" : "Cancel"}
                </button>
                <button onClick={handleCropConfirm} className="px-4 py-2 text-xs font-semibold bg-brand-purple text-white rounded-lg hover:bg-brand-purple-light transition-colors">
                  Crop & Upload
                </button>
              </div>
            </div>
